...
- [iOS] `Host` no longer applies SwiftUI's safe-area or keyboard insets by default. A host lays out like any React Native view: the content starts at the frame origin and the app handles insets and keyboard avoidance. Set the new `safeArea` prop on a host with scrolling content or text inputs, such as a `Form` or a `List`, to let SwiftUI handle them; with `matchContents`, the host grows by the applied insets so its frame still contains the content. `ignoreSafeArea` is deprecated and maps onto `safeArea`. ([#TBD](https://github.com/expo/expo/pull/TBD) by [@nishan](https://github.com/intergalacticspacehighway))
...
- [iOS] Views hosted in `RNHostView` now report their position from the host's own frame, so `measure()` and touch targets are correct when SwiftUI insets the content, for example on a `Host` with `safeArea`. ([#TBD](https://github.com/expo/expo/pull/TBD) by [@nishan](https://github.com/intergalacticspacehighway))
